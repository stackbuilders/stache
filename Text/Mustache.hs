...
-- Aeson.
--
-- __NOTE__: Beginning from the version 0.2.0, 'renderMustache' throws
-- exceptions when referenced key is not provided. Strictly speaking, this
-- is against the spec, but we must remember that the spec was written with
-- dynamically typed languages like Ruby and JavaScript in mind. Missing key
-- in 95% of cases is a programmer's mistake and it should be reported
-- (should fail “loudly”). If presence of a value is conditional, use
-- 'Maybe' (as you should in Haskell).
--
